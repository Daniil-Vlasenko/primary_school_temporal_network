...
Перейдем к визуализации всей сети, только теперь будет декорировать граф,
используя возможности библиотеки igraph. Каждый класс будем окрашивать в свой цвет.
Вершины учеников изобразим как круги, а вершины учителей - как квадраты. Размер
вершины будет пропорционален ее степени. Ширина ребра пропорционально ее весу.
```{r}
...
На этом графике хорошо заметно как учителя занимаются с одним или двумя классами,
не более.
...
E(network.c)$weight <- 1
network.c <- simplify(network.c)

class.size <- as.vector(table(V(network)$class))
plot(network.c, 
     vertex.size=5*sqrt(class.size),
     vertex.label=classes)


classes

V(network)$class
table(V(network)$class)
as.vector(table(V(network)$class))
class.size
