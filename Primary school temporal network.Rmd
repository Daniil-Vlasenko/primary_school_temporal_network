...
Так как у ребер есть атрибут Freq, который отражает силу связи инцидентных ребру вершин, то рассмотрим два два типа распределений: распределение степеней вершин и взвешенное распределение вершин (распределение сил вершин).
...
print(paste0("firts 10 degrees: ", toString(V(network)$degree[1:10])))

V(network)$wdegree <- strength(network, weights=E(network)$Freq)
print(paste0("firts 10 wdegree: ", toString(V(network)$wdegree[1:10])))
```
...
##### Взвешенное распределение степеней вершин
...
Изобразим матрицу корреляций степеней вершин, ячейка (i, j) которой демонстрирует вероятность связи вершин со степенями i и j:
```{r, message=FALSE, warning=FALSE}
library(reshape)
